...
def main():
    ...
    try:
        ...
        if read_only_catalog_grants:
            ...
            privileges = [g.get("privilege") for g in grants]
            has_read_props = any("NAMESPACE_READ_PROPERTIES" in p for p in privileges)
            has_table_list = any("TABLE_LIST" in p for p in privileges)
            has_table_read = any("TABLE_READ_DATA" in p for p in privileges)
            if has_read_props:
                print("      ✓ NAMESPACE_READ_PROPERTIES permission is granted")
            else:
                print("      ✗ NAMESPACE_READ_PROPERTIES missing")
                all_passed = False
            if has_table_list:
                print("      ✓ TABLE_LIST permission is granted")
            else:
                print("      ✗ TABLE_LIST missing")
                all_passed = False
            if has_table_read:
                print("      ✓ TABLE_READ_DATA permission is granted")
            else:
                print("      ✗ TABLE_READ_DATA missing")
                all_passed = False
        else:
        ...
        if read_write_catalog_grants:
            ...
            privileges = [g.get("privilege") for g in grants]
            has_create = any("TABLE_CREATE" in p for p in privileges)
            has_write = any("TABLE_WRITE_DATA" in p for p in privileges)
            has_read = any("TABLE_READ_DATA" in p for p in privileges)
            if has_create:
                print("      ✓ TABLE_CREATE permission is granted")
            else:
                print("      ✗ TABLE_CREATE missing")
                all_passed = False
            if has_write:
                print("      ✓ TABLE_WRITE_DATA permission is granted")
            else:
                print("      ✗ TABLE_WRITE_DATA missing")
                all_passed = False
            if has_read:
                print("      ✓ TABLE_READ_DATA permission is granted")
            else:
